...
- 複数 Insta360 カメラの SSID 登録（表示名・Wi-Fi パスワード・保存先）
...
- カメラごとのバックアップ先（必須）。同期情報は各保存先ルートの `.insta360-sync/` に配置
- バックアップ先フォルダ構造:
  - **オリジナル構造**: `{カメラの保存先}/{カメラ名}/DCIM/...`
  - **日付別**: `{カメラの保存先}/{YYYY-MM-DD}/{ファイル名}`（SD 上の階層は無視）
- 既存ファイルはスキップ（同名・同サイズ）
...
2. **設定** でカメラ（SSID・パスワード・保存先）を登録（保存先はカメラごとに必須）
